...
#    for N in [4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096]: # choose N even
    for N in [4, 8, 16, 32, 64, 128, 256, 512, 1024]: # choose N even
        ...
        Kemenytheo = 8 * epsilon ** 2 * (3.0 * M ** 2 - 1)  + 6 * epsilon * \
          (3 * M ** 2 + 4 * M -2.0) * p + (18 * M -5.0) * p ** 2
        Kemenytheo /= 6.0 * p * (epsilon + p) * (2 * epsilon + p) 
        if abs(2.0 * delta - p) < 0.0001: print(N, Kemeny.real, Kemenytheo, 'N, Kemeny , Kemenytheo')
        xvalues.append(N)
...
plt.show()
fig.savefig('FigureLiftedSquare.pdf')
