...
#    for N in [4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096]: # choose N even
    for M in [4, 8, 16, 32, 64, 128, 256, 512, 1024]: # choose N even (we only use M)
        epsilon = epsilontilde / M
        N = M * 2
        Table = []
        PiStat = {}
        Omega = {}
        for x in range(1, M + 1):
            ...
            if model == 'Wedge':
                if x <= M:
                    PiStat[(x, -1)] = 8.0 / N ** 2 * abs( (N + 1) / 2 - x) / 2.0
                    PiStat[(x,  1)] = 8.0 / N ** 2  * abs( (N + 1) / 2 - x) / 2.0
                    Omega[x] = p + epsilon
                else: 
                    PiStat[(x, -1)] = 0.0
        ...
        Kemenytheo = (4.0 / 3.0 * M - 1.0 / 2.0 - 1 / (3 * M) ) / p
...
plt.show()
fig.savefig('FigureLiftedWedge.pdf')
